...
public class ConstructionDefinitionMutable implements ConstructionDefinition {
	...
	public ConstructionProblem addProperty(Class<? extends PropertyGroup> group, Property<?> property, 
		...
		List<Name> allInNames = names.getAllInNames();
		...
		for (Name name : allInNames) {
			Property<?> conflictProp = propertiesByAnyName.get(name.getEffective());
			if (conflictProp != null) {
				ConstructionProblem.NonUniqueNames notUniqueName = new ConstructionProblem.NonUniqueNames(
					getGroupForProperty(conflictProp),
						conflictProp, group, property, name.getActual());
		...
		ConstructionProblem invalidDefault = 
				checkForInvalidDefaultValue(property, group, names.getCanonicalName().getActual());
		...
		canonicalNameByProperty.put(property, names.getCanonicalName().getActual());
		properties.add(property);


		for (Name n : allInNames) {
			propertiesByAnyName.put(n.getEffective(), property);
		}
