...
public class ConstructionDefinitionMutable implements ConstructionDefinition {
	...
	public ConstructionProblem addProperty(Class<? extends PropertyGroup> group, Property<?> property, 
		...
		List<EffectiveName> allInNames = names.getAllInNames();
		...
		for (EffectiveName name : allInNames) {
			Property<?> conflictProp = propertiesByAnyName.get(name.getEffectiveName());
		...
		for (EffectiveName a : names.getAllOutNames()) {
			if (! propertiesByExportName.containsKey(a.getActualName())) {
				propertiesByExportName.put(a.getActualName(), property);
			} else {
		...
		for (EffectiveName n : allInNames) {
			propertiesByAnyName.put(n.getEffectiveName(), property);
		}
