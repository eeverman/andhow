...
public interface SampleRestClientGroup extends PropertyGroup {
	...
	StrProp REST_HOST = StrProp.builder().mustMatchRegex(".*\\.usgs\\.gov") .nonNull().build();
	IntProp REST_PORT = IntProp.builder().nonNull().mustBeGreaterThanOrEqualTo(80).mustBeLessThan(10000).build();
	StrProp REST_SERVICE_NAME = StrProp.builder().defaultValue("query/").mustEndWith("/").build();
	StrProp AUTH_KEY = StrProp.builder().nonNull().build();
